...
# カテゴリ別のカウント（表示用）
COMMITTED_COUNT=$(echo "$COMMITTED_FILES" | grep -c -v '^$' || true)
UNSTAGED_COUNT=$(echo "$UNSTAGED_FILES" | grep -c -v '^$' || true)
STAGED_COUNT=$(echo "$STAGED_FILES" | grep -c -v '^$' || true)
UNTRACKED_COUNT=$(echo "$UNTRACKED_FILES" | grep -c -v '^$' || true)
