...
`template` が空でない場合、**テンプレートの見出し・構造を維持したまま**内容を埋める:

- ` ```text ... ``` ` のコードブロック内に書かれたプレースホルダ文（「このPRの概要を〜」「行った動作確認〜」など）を、実装内容を要約した日本語に置き換える
- 「概要」ブロック: **何を・なぜ**変更したかを簡潔に記述する。背景がある場合は補足する
- 「セルフレビュー」ブロック: 差分から推定した動作確認の観点・手順を記述する
- 箇条書き項目（`- a` / `- b` / `- c` などのプレースホルダ）は、差分から妥当な動作確認候補で置換する（例: `- ユニットテストを追加し通過を確認`）
- その他のセクション順序・見出しは変更しない
